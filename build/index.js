...
**\`record\`** — Write something back to your memory so it persists beyond this session. Use when a decision gets made, you learn something, or context shifts. Be journalistic.`);
    ...
    server.tool("transcript", "Retrieve a cleaned-up conversation transcript from the agent's main session for a given date. Returns user and assistant messages only — no tool calls, system messages, or errors. Output is a markdown transcript with timestamps and attribution.", {
        ...
        if (!/^\d{4}-\d{2}-\d{2}$/.test(targetDate)) {
            return {
                content: [{ type: "text", text: `Invalid date format: "${targetDate}". Use YYYY-MM-DD.` }],
                isError: true,
            };
        }
        const sessionId = await getActiveSessionId();
        if (!sessionId) {
            return {
                content: [{ type: "text", text: "_No active session found._" }],
            };
        }
        const sessionFile = join(SESSIONS_DIR, `${sessionId}.jsonl`);
        try {
            ...
            if (!messages.length) {
                return {
                    content: [{ type: "text", text: `_No conversation messages found for ${targetDate}._` }],
                };
            }
            ...
            return {
                content: [{ type: "text", text: `_Error reading session transcript: ${e}_` }],
                isError: true,
            };
        }
    });
    ...
    server.tool("record", "Write a memory, observation, or note back into the agent's workspace. Use when you learn something worth remembering — a decision made, a pattern noticed, a lesson learned. This is how tool sessions feed back into the agent's continuous memory.", {
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: `Recorded ${cat} to memory/${dateStr}.md` }],
            };
        }
        catch (e) {
            return {
                content: [{ type: "text", text: `Failed to record: ${e}` }],
                isError: true,
            };
        }
    });
